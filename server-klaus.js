...
const leagues = [
  ...
  {
    path: "https://datahub.io/sports-data/italian-serie-a/datapackage.json",
    name: "SerieA",
  },
  {
    path: "https://datahub.io/sports-data/spanish-la-liga/datapackage.json",
    name: "LaLiga",
  },
];
var stats = {};
...
app.get("/probabilities", function (req, res) {
  ...
  console.log(
    "Calculating probabilities for a match between",
    team1,
    "and",
    team2,
    "..."
  ...
  if (stats[req.query.league][team1] == undefined) {
    ...
    var grid = KlausMath.getGrid(
      stats[req.query.league][team1].homeATK,
      stats[req.query.league][team1].homeDEF,
      stats[req.query.league][team2].awayATK,
      stats[req.query.league][team2].awayDEF
    );
    console.log(
      "Sending probabilities for a match between",
      team1,
      "and",
      team2
    );
    ...
    if (dataset.resources[id]._descriptor.name === "season-1819_json") {
      const writeStream = fs.createWriteStream(
        "data/" +
          league.name +
          "-" +
          dataset.resources[id]._descriptor.name.replace("_", ".")
      );
  ...
  if (fs.existsSync(path)) {
    ...
    for (var j = 0; j < leagueJsonObj.length; j++) {
      ...
      if (teams.hasOwnProperty(homeTeam)) {
        ...
        teams[homeTeam] = {
          goalsScoredHome: homeGoal,
          goalsTakenHome: awayGoal,
          nbMatchHome: 1,
          goalsScoredAway: 0,
          goalsTakenAway: 0,
          nbMatchAway: 0,
      ...
      if (teams.hasOwnProperty(awayTeam)) {
        ...
        teams[awayTeam] = {
          goalsScoredAway: awayGoal,
          goalsTakenAway: homeGoal,
          nbMatchAway: 1,
          goalsScoredHome: 0,
          goalsTakenHome: 0,
          nbMatchHome: 0,
